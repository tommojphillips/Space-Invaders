...
static int load_rom() {
	if (read_file_into_buffer("pv01", invaders.mm.rom + 0x000, 0x800) != 0) {
		return 1;
	}
	if (read_file_into_buffer("pv02", invaders.mm.rom + 0x800, 0x800) != 0) {
		return 1;
	}
	if (read_file_into_buffer("pv03", invaders.mm.rom + 0x1000, 0x800) != 0) {
		return 1;
	}
	if (read_file_into_buffer("pv04", invaders.mm.rom + 0x1800, 0x800) != 0) {
		return 1;
	}
	if (read_file_into_buffer("pv05", invaders.mm.rom + 0x2000, 0x800) != 0) {
		return 1;
	}
...
uint8_t invaderspt2_read_byte(uint16_t address) {
	if (address < 0x2000) {
...
void invaderspt2_write_byte(uint16_t address, uint8_t value) {
	if (address < 0x2000) {
...
int invaderspt2_init() {
	...
	i8080_init(&cpu);
	cpu.read_byte = invaderspt2_read_byte;
	cpu.write_byte = invaderspt2_write_byte;
	cpu.read_io = invaderspt2_read_io;
	cpu.write_io = invaderspt2_write_io;
